...
I0-I10 implementation work is merged. The I7 acceptance audit found gaps; I8 addressed the checkout-flow gaps and added direct README coverage evidence. The [acceptance report](docs/i7-final-acceptance.md) records that audit, the I8 deployment, and its remaining verification limits. [PR #20](https://github.com/teraspace/payment-flow-lab/pull/20) added typed result flows for checkout and payment commands; its API image `i9-dc226ce` is deployed on ECS task-definition revision 6. [PR #23](https://github.com/teraspace/payment-flow-lab/pull/23) merged I10 as `e0f4084`; the I10 release record below covers its frontend deployment.

The API's application services depend on persistence and payment-gateway ports; PostgreSQL SQL and provider HTTP calls live in adapters. See the [architecture and consistency boundaries](docs/hexagonal-architecture.md). I12 architecture work is under review and is not part of the deployed application yet.
