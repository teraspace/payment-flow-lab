...
- Twelve Jest integration cases passed against real PostgreSQL connections, including final-unit contention, concurrent same-key replay, cross-session isolation, payload conflict, failed-stock retry, reservation expiry, price snapshot, rollback after an injected intermediate database error, idempotent session reuse, and cleanup of expired sessions without checkouts. Both migrations applied and rolled back in the isolated test database.
