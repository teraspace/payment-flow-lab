...
| Guest/customer identity for idempotency scope | Server-issued anonymous session; no account system in baseline | User approved a 30-day session lifetime on 2026-09-24. The I2 implementation uses a 256-bit random opaque token in an `HttpOnly; SameSite=Lax` cookie (`Secure` in production); only the token hash is stored. Cookie mechanics remain an implementation proposal for the I2 review. Idempotency keys are SHA-256 hashed and scoped to session + operation. |
...
Still open for later contract refinement: the I2 guest-session cookie mechanics, user-facing cancellation control, unresolved-attempt escalation threshold, and webhook receipt retention. Resolve each before the iteration that depends on it and record any approval or revision.
...
- The user approved a 30-day guest-session lifetime on 2026-09-24. Cookie mechanics remain an implementation proposal for review; no payment-provider call or payment-attempt route is added in I2.
...
- The user approved redaction of customer name/email, delivery recipient/address, and the PII-derived request fingerprint 30 days after checkout creation on 2026-09-24. A five-minute scheduler uses PostgreSQL transactions and locked batches; it preserves non-PII checkout and inventory history. The selected policy is explicit before I2 deployment.
