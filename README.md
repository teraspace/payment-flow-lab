...
Iterations 0 through I3 are complete. I2 is merged in PR #8, and I3 is merged in PR #10 and deployed at [the demo URL](https://d2ump7odi96dfi.cloudfront.net). I3 adds durable payment attempts, a provider adapter, signed webhooks, reconciliation, and inventory/fulfillment transitions; the deployed API uses the immutable image `i3-23f9aa2`. I4 is implemented on `feat/react-checkout` and remains in draft PR #12; it adds the React catalog, delivery form, server-priced order summary, sandbox-only card tokenization, payment state/recovery, guest checkout recovery, and same-origin API relays for acceptance metadata and browser-encrypted card tokenization. It is not merged or deployed; the live React app remains the I1 shell. The API rejects production payment configuration, and no sandbox credentials are stored in the repository. Local sandbox smoke evidence: a declined test attempt reconciled to `DECLINED`; a subsequent approved-card attempt on that checkout became `UNKNOWN_OUTCOME` without a provider transaction ID and remains unresolved with its stock held; a separate approved test checkout reached `PAID`, committed inventory, and created `READY` fulfillment. No real card or real charge was used. Local lint, typecheck, build, and 49 API tests pass. Review the [requirements and scorecard](docs/requirements-traceability.md), [API contract](docs/api-contract.md), [payment and inventory lifecycle](docs/payment-lifecycle.md), [payment operations runbook](docs/payment-operations.md), and [open decisions](docs/open-decisions.md).
