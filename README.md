...
Iterations 0 through I3 are complete. I2 is merged in PR #8, and I3 is merged in PR #10 and deployed at [the demo URL](https://d2ump7odi96dfi.cloudfront.net). I3 adds durable payment attempts, a provider adapter, signed webhooks, reconciliation, and inventory/fulfillment transitions; the deployed API uses the immutable image `i3-23f9aa2`. I4 is in progress on `feat/react-checkout`: it adds the React catalog, delivery form, server-priced order summary, sandbox-only card tokenization, payment state/recovery, guest checkout recovery, and a same-origin API route for sandbox acceptance documents. It is not merged or deployed; the live React app remains the I1 shell. Committed sandbox configuration is blank by default, and the browser/API reject production payment configuration. A local metadata-only check used test credentials from the challenge document through process environment; no credential is stored in the repository. No card has been tokenized and no payment transaction has been submitted; explicit consent in the local checkout remains pending. Review the [requirements and scorecard](docs/requirements-traceability.md), [API contract](docs/api-contract.md), [payment and inventory lifecycle](docs/payment-lifecycle.md), [payment operations runbook](docs/payment-operations.md), and [open decisions](docs/open-decisions.md).
