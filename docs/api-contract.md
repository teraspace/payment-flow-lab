...
| `POST /api/v1/webhooks/payment-events` | Verifies event environment, timestamp, dynamic signed properties, checksum, reference, transaction ID, amount, and currency. Persists only a minimal receipt and applies the state transition atomically before returning `200`. Duplicate valid events return `200` without repeating effects. Invalid shape/signature returns `400`; an unconfigured verifier returns `503`; storage failures return `500` so delivery can retry. |
