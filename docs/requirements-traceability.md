...
The current official provider documentation was rechecked on 2026-09-25. It describes browser-side card tokenization, two separate consent tokens, transaction amounts in integer centavos, an initial successful creation state of `PENDING`, terminal states including `APPROVED`, `DECLINED`, `VOIDED`, and `ERROR`, server-side lookup by provider transaction ID, and signed events. It requires a unique reference but does not establish a remote idempotency guarantee. Therefore an ambiguous timeout or duplicate-reference response is not treated as proof that a charge succeeded or failed. Current standard sandbox keys use `pub_test_`/`prv_test_`; the separate challenge-provided staging profile was verified only with read-only metadata calls.

I3 verifies these contract details with unit tests and a deterministic provider double against real PostgreSQL. I4 follows the published browser tokenization and acceptance-document flow, restricts input to published sandbox test cards, and gates the payment adapter to test configuration. No provider secrets or provider-specific URL are kept in this public project documentation. Local test credentials were passed only through the running process environment; acceptance metadata and the encryption key returned successfully. The browser displays both required consent controls, but no consent was accepted, no card was tokenized, and no payment transaction was submitted. The full sandbox outcome/retry pass remains pending.
...
- Payment capture is sandbox-only by design: the browser and API reject production configuration, the card input accepts only published sandbox test PANs, and no real card or credential is checked in. The committed environment example stays blank. A local run used challenge test credentials from process environment for read-only acceptance/key requests; none were written to the repository.
...
- This implementation has not been merged or deployed. Local lint, typecheck, build, `git diff --check`, and 44 API tests across five suites passed; hosted `quality`, `database-migrations`, and `infrastructure` checks passed on PR #12 head `fff09ba`.
- Browser smoke covered catalog, delivery form, server-priced checkout creation, and recovery after refresh. It used only synthetic `.test` contact and delivery data. The sandbox consent documents now load through the local API because the challenge metadata endpoint does not expose browser CORS headers.
- The current local checkout shows both consent controls. No consent has been accepted, no payment card was entered or tokenized, and no transaction was submitted. Approval/decline, retry, and reconciliation UI evidence remains pending; no full-flow scorecard points are claimed yet.
