...
| Payment attempt | `PENDING`, `FAILED_LOCAL`, `REJECTED_NO_TRANSACTION`, `UNKNOWN_OUTCOME`, `APPROVED`, `DECLINED`, `ERROR`, `VOIDED` | New attempts are committed as `PENDING` before the provider call. While no provider response has been recorded, the API returns `dispatching: true`; this transport-progress marker is not a second domain state. `FAILED_LOCAL` means the adapter proves no request bytes were sent. `REJECTED_NO_TRANSACTION` records an explicit provider rejection before transaction creation. Neither consumes a provider charge retry. Repeated observations cannot downgrade a newer result; a newer contradiction after approval triggers manual review. A deliberate retry is a new resource with a new local key, provider reference, and card token. |
...
stateDiagram-v2
    ...
    RESERVED --> PAYMENT_PENDING: commit local attempt as PENDING
    PAYMENT_PENDING --> PAYMENT_PENDING: provider response recorded as PENDING
    RESERVED --> UNKNOWN_OUTCOME: timeout after send may have begun
    PAYMENT_PENDING --> RESERVED: mark attempt FAILED_LOCAL when no request bytes were sent
    PAYMENT_PENDING --> RESERVED: provider rejects request before transaction creation
...
3. The frontend obtains the current terms and personal-data authorization tokens, shows both applicable consent texts, and obtains explicit acceptance for both. It fetches the provider's test encryption key through the API, encrypts card details in the browser, then sends only the compact encrypted payload through the API relay to the sandbox tokenization endpoint. The API never receives PAN or CVC in plaintext and does not persist or log the encrypted payload or returned card token. Each deliberate payment attempt uses a fresh card token.
4. A short PostgreSQL transaction checks checkout/session ownership, reservation eligibility, attempt history and idempotency, inserts the attempt as `PENDING` with a unique per-attempt reference, changes the checkout to `PAYMENT_PENDING`, then commits. Only after that transaction has closed does the API call the provider, with the private API key held server-side. The API calculates the integrity signature from the stored amount/reference/currency; it does not accept a client total or client-generated signature as authoritative. Until a response is persisted, the attempt view carries `dispatching: true`; after any response or classified failure it becomes false. This separates domain status from transport progress.
...
The docs require unique transaction references but do not describe that field as an idempotency key. A duplicate-reference error is not a replayed success. The documented card-transaction status lookup requires a provider transaction ID; an unknown outcome with no ID can be resolved by a verified webhook matched on reference or by operational review, but the application cannot query it by inventing a reference endpoint. The adapter treats transport failures and ambiguous non-`400`/`401` responses conservatively as unknown.
