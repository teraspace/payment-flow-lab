...
I0-I12 implementation work is merged. The I7 acceptance audit found gaps; I8 addressed the checkout-flow gaps and added direct README coverage evidence. The [acceptance report](docs/i7-final-acceptance.md) records that audit, the I8 deployment, and its remaining verification limits. [PR #20](https://github.com/teraspace/payment-flow-lab/pull/20) added typed result flows for checkout and payment commands. [PR #23](https://github.com/teraspace/payment-flow-lab/pull/23) merged I10 as `e0f4084`; the I10 release record below covers its frontend deployment. [PR #26](https://github.com/teraspace/payment-flow-lab/pull/26) merged I12 as `12f526e`; the API image `i12-12f526e` now runs on ECS task-definition revision 7.

The API's application services depend on persistence and payment-gateway ports; PostgreSQL SQL and provider HTTP calls live in adapters. See the [architecture and consistency boundaries](docs/hexagonal-architecture.md). I12 is deployed to the API; the React frontend and AWS infrastructure remain unchanged. The I12 rollout required no schema migration or sandbox payment. Public app, liveness, readiness (`database: ok`), catalog, and Swagger checks returned HTTP 200 after ECS stabilized at 1/1. The targeted post-deploy Terraform plan reported no API changes. The full plan's unrelated generated web-asset differences were not applied.

The [deployed application](https://d2ump7odi96dfi.cloudfront.net) connects the CloudFront frontend to the Fargate API; the public [Swagger UI](https://d2ump7odi96dfi.cloudfront.net/api/v1/docs) documents its endpoints. PR #23's I10 frontend, favicon, and CloudFront response-header policy are deployed. Public checks returned HTTP 200 for the app, API readiness, and Swagger; CloudFront reports `Deployed`, and the static response includes CSP, HSTS, `nosniff`, frame denial, and no-referrer headers. Production browser checks passed in Chrome, Firefox, and WebKit at three mobile/desktop viewport sizes. [PR #21](https://github.com/teraspace/payment-flow-lab/pull/21) recorded I9 API deployment evidence, and [PR #22](https://github.com/teraspace/payment-flow-lab/pull/22) added the ER diagram below. No sandbox payment was submitted during I9 or I10 deployment verification. Earlier I4 sandbox evidence includes approved, declined, pending/unknown, and refresh-recovery paths; no real card or charge was used.
