...
I0 through I4 are complete and merged; the AWS deployment (planned as I6) was completed after I4. PR #12 merged the React checkout and PR #13 added scoped sandbox-secret injection for ECS. The [deployed application](https://d2ump7odi96dfi.cloudfront.net) serves the checkout with its Fargate API; on 2026-09-25 read-only smoke checks returned `200` for `/`, `/api/v1/health/live`, `/api/v1/health/ready` (`database: ok`), `/api/v1/products`, and `/api/v1/docs`. I4 sandbox evidence includes approved, declined, pending/unknown, and refresh-recovery paths; no real card or charge was used. I5 is under review in [draft PR #14](https://github.com/teraspace/payment-flow-lab/pull/14) from `feat/reliability-scorecard`; local gates and hosted `quality`, `database-migrations`, and `infrastructure` checks passed. Coverage, accessibility, performance, known gaps, and reproducible commands are recorded in the [reliability scorecard](docs/reliability-scorecard.md). Review the [requirements and 100-point rubric](docs/requirements-traceability.md), [API contract](docs/api-contract.md), [payment and inventory lifecycle](docs/payment-lifecycle.md), [payment operations runbook](docs/payment-operations.md), and [open decisions](docs/open-decisions.md).
