...
I0 through I5 are complete and merged; the AWS deployment (planned as I6) was completed after I4, with the I5 frontend update deployed on 2026-09-25. PR #12 merged the React checkout, PR #13 added scoped sandbox-secret injection for ECS, and [PR #14](https://github.com/teraspace/payment-flow-lab/pull/14) merged the I5 reliability scorecard. The [deployed application](https://d2ump7odi96dfi.cloudfront.net) serves the checkout with its Fargate API; after I5 deployment, read-only smoke checks returned `200` for the app, both API health endpoints, the catalog, API docs, and the new JS/CSS bundles. Terraform changed only the versioned frontend objects in the existing private S3 bucket; ECS, RDS, IAM, secrets, networking, and CloudFront distribution were unchanged. I4 sandbox evidence includes approved, declined, pending/unknown, and refresh-recovery paths; no real card or charge was used. I5 coverage, accessibility, local Lighthouse measurements, known gaps, and reproducible commands are recorded in the [reliability scorecard](docs/reliability-scorecard.md). Review the [requirements and 100-point rubric](docs/requirements-traceability.md), [API contract](docs/api-contract.md), [payment and inventory lifecycle](docs/payment-lifecycle.md), [payment operations runbook](docs/payment-operations.md), and [open decisions](docs/open-decisions.md).
