...
Iterations 0 through I4 are merged. I2 is merged in PR #8, I3 is merged in PR #10, and I4 is merged in PR #12 as `0e047f7`. The existing demo remains at [the deployed URL](https://d2ump7odi96dfi.cloudfront.net), currently on the I3 API and I1 shell; I4 has not yet been deployed. I4 adds the React catalog, delivery form, server-priced order summary, sandbox-only card tokenization, payment state/recovery, guest checkout recovery, and same-origin API relays for acceptance metadata and browser-encrypted card tokenization. Local sandbox evidence: a declined test attempt reconciled to `DECLINED`; its one retry became `UNKNOWN_OUTCOME` without a provider transaction ID and remains held; a separate approved test checkout reached `PAID`, committed inventory, and created `READY` fulfillment. No real card or real charge was used. AWS deployment is pending because the CLI session expired and the ECS task definition still needs secure sandbox-secret wiring. Local lint, typecheck, build, and 49 API tests passed; I4's merge commit passed hosted CI. Review the [requirements and scorecard](docs/requirements-traceability.md), [API contract](docs/api-contract.md), [payment and inventory lifecycle](docs/payment-lifecycle.md), [payment operations runbook](docs/payment-operations.md), and [open decisions](docs/open-decisions.md).
